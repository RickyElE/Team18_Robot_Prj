...
// #include "../include/libcam2_node/libcam2web.h"
#include "../include/libcam2_node/libcam2opencv.h"
...
public:
    ...
    // // 設置攝像頭參數
    // void setCameraSettings(int port, const Libcam2OpenCVSettings& settings) {
    //     camera_port_ = port;
    ...
    // bool initCamera() {
    //     try {
    //         std::cout << "初始化攝像頭服務器..." << std::endl;
    //         camera_server_ = new LibCam2Web(camera_port_);
    //         return true;
    //     } catch (const std::exception& e) {
    //         std::cerr << "攝像頭初始化錯誤: " << e.what() << std::endl;
    //         return false;
    //     }
    // }
    ...
    //             if (camera_server_) {
    //                 camera_server_->start(camera_settings_);
    //             }
    //         } catch (const std::exception& e) {
    //             std::cerr << "攝像頭服務錯誤: " << e.what() << std::endl;
    //             running_ = false;
    //         }
    //     });
    //     camera_thread_.detach(); // 讓攝像頭服務在後台運行
    // }
        ...
        while(running_){
            ...
            if (ws_connected_) {
                try { 
                    ...
                    // std::vector<uchar> &jpg = latestFrame; // 假设它已经是 JPEG 编码
                    // std::vector<uint8_t> full_packet;
                    // // 加入 JSON 长度（大端）
                    // full_packet.push_back((json_len >> 24) & 0xFF);
                    // full_packet.push_back((json_len >> 16) & 0xFF);
                    // full_packet.push_back((json_len >> 8) & 0xFF);
                    // full_packet.push_back((json_len) & 0xFF);

                    // // 加入 JSON 本体
                    // full_packet.insert(full_packet.end(), json_str.begin(), json_str.end());

                    // // 加入 JPEG 图像
                    // full_packet.insert(full_packet.end(), jpg.begin(), jpg.end());

                    // // 一次性发送
                    // ws_server_.send(ws_hdl_, full_packet.data(), full_packet.size(), websocketpp::frame::opcode::binary);
                    ws_server_.send(ws_hdl_, msg.dump(), websocketpp::frame::opcode::text);
                    // ws_server_.send(ws_hdl_,latestFrame.data(), latestFrame.size(), websocketpp::frame::opcode::binary);
    ...
    void stop(){
        ...
        motor.cleanServoAngle(CAMERA_SERVO_CHANNEL);
        // http_server_.stop();
        camera.stop();
        motor.Stop();
        cpuTempture.Stop();
